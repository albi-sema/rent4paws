...
export default function SearchHeader({ initialParams = {} }) {
  const router = useRouter();
  const today = new Date().toISOString().split("T")[0];

  // Initialize state from initialParams if provided.
  const [pickupPlace, setPickupPlace] = useState(initialParams.pickupPlace || "Tirana");
  const [pickupDate, setPickupDate] = useState(initialParams.pickupDate || today);
  const [pickupTime, setPickupTime] = useState(initialParams.pickupTime || "09:00");
  const [returnDate, setReturnDate] = useState(initialParams.returnDate || today);
  const [returnTime, setReturnTime] = useState(initialParams.returnTime || "09:00");
      ...
      <div className="max-w-4xl mx-auto mt-8 px-4">
        ...
        >
          <div className="flex-1 md:max-w-[150px]">
            <label htmlFor="pickup-place" className="block font-semibold text-gray-700 mb-1">
              {t.formCity}
            </label>
            <select
              id="pickup-place"
          ...
          <div className="flex-1">
            <label htmlFor="pickup-date" className="block font-semibold text-gray-700 mb-1">
              {t.formPickupDate}
            </label>
            <input
              type="date"
          ...
          <div className="flex-1">
            <label htmlFor="pickup-time" className="block font-semibold text-gray-700 mb-1">
              {t.formPickupTime}
            </label>
            <input
              type="time"
          ...
          <div className="flex-1">
            <label htmlFor="return-date" className="block font-semibold text-gray-700 mb-1">
              {t.formReturnDate}
            </label>
            <input
              type="date"
          ...
          <div className="flex-1">
            <label htmlFor="return-time" className="block font-semibold text-gray-700 mb-1">
              {t.formReturnTime}
            </label>
            <input
              type="time"
